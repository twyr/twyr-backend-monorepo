 ...
 */
export class Contacts extends SystemAdminBaseMiddleware {
	// #region Constructor
	/**
	 * @category REST API Server/Domains/System Admin
	 * @subcategory Profile/Middlewares
	...
	async #createContacts({ user, data }) {
		return this?._createUserEntity?.({
			model: { type: 'relational', name: 'system-admin-contact' },
			user,
			payload: data,
	...
	async #readContacts({ user, contactId }) {
		return this?._readUserEntity?.({
			...
			entityId: contactId,
			relationships: '[user, contactType]',
			serializeType: 'system_admin_contact'
		});
	}
	...
	async #deleteContacts({ user, contactId }) {
		const returnValue = await this?._deleteUserEntity?.({
			...
			user,
			entityId: contactId,
			eventName: 'SYSTEM_ADMIN::CONTACT_DELETED'
		});
