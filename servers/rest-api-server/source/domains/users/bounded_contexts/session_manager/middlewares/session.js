...
export class Session extends BaseMiddleware {
	...
	async #generateOtp({ username, locale }) {
		...
		cacheMulti?.set?.(`user-otp-${username}`, secureOtp);
		cacheMulti?.expire?.(`user-otp-${username}`, 600);
		...
		const otpMessage = await i18nRepository?.translate?.(
			'USERS::SESSION_MANAGER::OTP_MESSAGE_SMS',
			locale,
			{
				otp: secureOtp,
				expiryTime: expiryTime
		...
		const responseMessage = await i18nRepository?.translate?.(
			'USERS::SESSION_MANAGER::OTP_MESSAGE_RESPONSE',
			locale
		);
	...
	async #validateOtp({ username, password, locale }) {
		...
		const storedOtp = await this?._executeWithBackOff?.(async () => {
			return cacheRepository?.get?.(`user-otp-${username}`);
		});
	 ...
	 */
	async #login({ user }) {
		this?.domainInterface?.eventEmitter?.emit?.('USER_LOGIN', {
			userId: user?.id
		});
	...
	async #logout({ userId, userName, locale }) {
		...
		const responseMessage = await i18nRepository?.translate?.(
			'USERS::SESSION_MANAGER::LOGOUT_MESSAGE',
			locale,
			{ userName: userName }
		);
