 ...
 *
 * @description
 * Sets up local authentication strategy for system administrators.
 */
			...
			async (request, username, password, callback) => {
				try {
					let userRecord = await databaseRepository?.raw?.(
						`SELECT * FROM system_admins WHERE mobile_no = ?`,
						[username]
					);
					...
					const otpNumber = await cacheRepository?.get?.(
						`system-admin-otp-${username}`
					);

					if (!otpNumber || otpNumber !== password) {
						const userError = new Error(
